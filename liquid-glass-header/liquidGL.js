...
(() => {
  ...
  class liquidGLRenderer {
      ...
      const fsSource = `
        ...
        void main(){
          ...
          if (u_frost > 0.0) {
              ...
              // Base filtered color
              vec4 baseFiltered = texture2D(u_tex, sampleUV);
              baseFiltered += texture2D(u_tex, sampleUV + vec2( texel.x, 0.0));
              baseFiltered += texture2D(u_tex, sampleUV + vec2(-texel.x, 0.0));
              baseFiltered += texture2D(u_tex, sampleUV + vec2(0.0,  texel.y));
              baseFiltered += texture2D(u_tex, sampleUV + vec2(0.0, -texel.y));
              baseFiltered /= 5.0;

              // Chromatic aberration: gently separate RGB channels along radial direction
              float ca = u_chromaticAberration;
              if (ca > 0.0) {
                vec2 dir = normalize(offset + 1e-6);
                vec2 rUV = sampleUV + dir * ( ca);
                vec2 gUV = sampleUV;
                vec2 bUV = sampleUV - dir * ( ca);
                vec4 rS = texture2D(u_tex, rUV);
                vec4 gS = texture2D(u_tex, gUV);
                vec4 bS = texture2D(u_tex, bUV);
                // Blend with baseFiltered to avoid harsh color fringing in the center
                float edgeStrength = smoothstep(0.05, 0.8, edge);
                vec3 caMix = vec3(rS.r, gS.g, bS.b);
                refrCol = vec4(mix(baseFiltered.rgb, caMix, edgeStrength), baseFiltered.a);
              } else {
                refrCol = baseFiltered;
              }
            }
    ...
    _renderLens(lens) {
      ...
      const chroma = Math.max(
        ...
        Math.min(
          0.02,
          lens.options.chromaticAberration !== undefined
            ? lens.options.chromaticAberration
            : 0.0
        )
      );
      gl.uniform1f(this.u.chromaticAberration, chroma);
  ...
  class liquidGLLens {
    constructor(renderer, element, options) {
      ...
      // Allow interaction with the lens element (e.g., header links)
